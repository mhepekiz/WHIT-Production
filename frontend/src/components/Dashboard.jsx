...
const Dashboard = () => {
  ...
  const fetchFunctions = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://staging.whoishiringintech.com/api'}/functions/`);
      if (response.ok) {
        const data = await response.json();
        setFunctions(data);
      }
  ...
  const fetchWorkEnvironments = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://staging.whoishiringintech.com/api'}/work-environments/`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleResumeUpload = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://staging.whoishiringintech.com/api'}/accounts/profile/upload_resume/`, {
        method: 'POST',
        headers: {
          'Authorization': `Token ${token}`,
        },
